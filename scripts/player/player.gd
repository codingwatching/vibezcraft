...
func take_damage(
	...
	if Game.is_loading:
		return
	# Creative is invulnerable, full stop. Individual sources used to opt
	# out one at a time (fall in _land_from_fall, drowning in _tick_air),
	# which left mobs, lava, fire, cactus and explosions still hurting a
	# creative player. One gate here is the guarantee, and the per-source
	# checks stay as cheap early-outs that skip the bookkeeping entirely.
	if creative_mode:
		return
