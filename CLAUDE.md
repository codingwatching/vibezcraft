...
- **Natural spawn:** `natural_mob_spawner.gd` — per-tick random-cell sample around player. Hostile pool `{zombie, skeleton, spider, creeper}` gated by night + light ≤ 7 + 24-128 m XZ band. Slime separate path (slime-chunk + Y ≤ 40, no light gate, no night gate). Hostile cap scales with loaded chunks (vanilla bg.java `70 × chunks / 256`, clamped to 70 — so Normal/Far get the full 70, Tiny gets ~6). `passive_spawner.gd` handles pig/cow/sheep/chicken.
...
- **Gamepad support (desktop + web):** Bedrock-parity bindings registered in `InputActions._register_gamepad_defaults` (left stick = analog movement through the same actions `Input.get_vector` reads; RT/LT = attack/use; A jump; B sneak/dismount/fly-down; X+Y inventory; LB/RB hotbar; D-pad up/down = perspective/drop; Start pause; L3/R3 unbound — Alpha has no sprint or slow-descend). Right-stick look is polled per-frame in `player.gd::_poll_pad_look` (deadzone + quadratic curve). Key/mouse rebinds and saved overrides erase ONLY key/mouse events (`_erase_key_mouse_events`) so they never strip the pad layer — guarded by `tests/test_input_actions.gd`. Menus/inventory stay pointer-driven.
...
- **Web/mobile-web perf gates** (all platform-gated — desktop native behavior is untouched; don't "unify" them away): web = no sun shadow map (`day_night_driver.gd`) + physics catch-up capped at 3 steps/frame (`project.godot`); mobile web additionally = 3D renders at ~1.5× CSS resolution (`game.gd` scaling_3d), ambient scan 250 cells (`ambient_fx.gd`), random ticks 12/chunk (`blocks.gd`), Fast clouds + Tiny render distance first-boot defaults (`settings_menu.gd`).
...
scripts/
  ...
  dev/                        # pre-commit.sh, install-hooks.sh, serve_web.py,
                              #  perf_probe.gd (timing rings), perf_watchdog.gd (sustained
                              #  low-fps line in DebugLog), debug_log.gd,
                              #  nether_trip_probe.gd (headless round trip),
                              #  seam_probe.gd (windowed chunk-seam crack count)
...
**Mob-vs-mob soft push.** `MobBase._apply_mob_separation` runs from `_physics_process` for NEAR/MID mobs. Symmetric per-pair impulse, vanilla `MOB_PUSH_STRENGTH = 0.05`. Without this, mobs would clip clean through each other since `VoxelCollider` only checks block cells. Skipped for FAR mobs (clipping invisible at distance) — preserves the early-out savings. Pair lookup goes through a static spatial hash (`_sep_grid`, 2 m cells, rebuilt once per physics frame) instead of an O(n²) sweep of `_active_mobs` — the n² version measured as the dominant frame cost at ~20 mobs on throttled wasm. Keep `_SEP_CELL > _MOB_PUSH_QUICK_REJECT` or the 3×3 neighborhood misses valid pairs.
